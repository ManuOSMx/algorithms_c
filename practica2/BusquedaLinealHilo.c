...
//ESCOM-IPN
//Medición de tiempo de Algoritmo de Busqueda Lineal con Hilos
//Compilación: "gcc BusquedaLinealHilo.c tiempo.c -o hilo"
//Ejecución: "./hilo" (Linux)
//compliación opción 2: ./InstruccionesLH.bash
//
//*****************************************************************
...
{	
	...
	if(busquedaLineal(A,inicio,fin,B[j])!=-1){
...
int busquedaLineal(int A[], int inicio, int final, int clave){
	...
	for(i=inicio; i<final; i++){
		if(A[i]==clave){ //se hace una comparacion del numero a buscar con cada posicion de arreglo A
			return i;
		}
	}
